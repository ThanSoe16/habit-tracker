...
export const useFocusSessionStore = create<FocusSessionStore>()(
  persist(
    (set, get) => ({
      ...
      start: (durationSeconds, selectedApps) => {
        ...
        void digitalWellbeingService
          .startFocusSession(id, startedAt.toISOString(), safeDuration, selectedApps)
      ...
      pause: () => {
        ...
        void digitalWellbeingService
          .updateFocusSession(session.id, {
            ...
            completedDurationSeconds: completed,
      ...
      resume: () => {
        ...
        const updated = {
          ...
          pausedAt: null,
          endsAt: new Date(new Date(session.endsAt).getTime() + pausedFor).toISOString(),
        };
        set({ activeSession: updated });
        void digitalWellbeingService
          .updateFocusSession(session.id, {
            ...
            completedDurationSeconds: Math.max(
              ...
              session.plannedDurationSeconds -
                Math.ceil(
                  (new Date(session.endsAt).getTime() - new Date(session.pausedAt).getTime()) /
                    1000,
                ),
            ),
      ...
      extend: (seconds) => {
        ...
        const updated = {
          ...session,
          endsAt: new Date(new Date(session.endsAt).getTime() + extra * 1000).toISOString(),
          plannedDurationSeconds: session.plannedDurationSeconds + extra,
        };
        set({ activeSession: updated });
        void digitalWellbeingService
          .updateFocusSession(session.id, {
        ...
        void digitalWellbeingService
          .updateFocusSession(session.id, {
            ...
            completedDurationSeconds: completed
              ? session.plannedDurationSeconds
              : Math.min(session.plannedDurationSeconds, elapsed),
            pauseCount: session.pauseCount,
            endedAt,
          })
          .catch(console.warn);
    ...
    { name: 'digital-wellbeing-focus-session', skipHydration: true },
  ),
);
