...
export type MoodTab = 'today' | 'history';

interface MoodSidebarProps {
  onSelectTab?: (tab: MoodTab) => void;
}

export function MoodAppSidebar({ onSelectTab }: MoodSidebarProps) {
      ...
      items: [
        {
          ...
          icon: BarChart2,
          action: () => {
            router.push('/mood-generals/reports');
            setOpenMobile(false);
          },
        ...
        },
        {
          ...
          icon: Settings,
          action: () => {
            router.push('/mood-generals/settings');
            setOpenMobile(false);
          },
    ...
    <Sidebar className="border-r border-gray-100 dark:border-zinc-800/80 bg-[#fffefb] dark:bg-[#12161f] text-gray-900 dark:text-zinc-100 p-0">
      ...
      <SidebarHeader className="p-5 border-none bg-transparent">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-black text-xl shadow-md shadow-indigo-600/30">
            <Smile className="w-5 h-5 fill-white stroke-none text-indigo-600" />
          </div>
