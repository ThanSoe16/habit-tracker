...
interface MyHabitsPageProps {
  defaultTab?: 'habit' | 'task';
}

export default function MyHabitsPage({ defaultTab = 'habit' }: MyHabitsPageProps) {
    ...
    <div className="min-h-screen bg-background dark:bg-zinc-950 flex flex-col w-full max-w-lg mx-auto pb-28">
      ...
      <header className="flex justify-between items-center px-4 py-3">
        ...
        <h1 className="text-lg font-black text-gray-900 dark:text-white tracking-tight">
          {defaultTab === 'task' ? 'One-Time Tasks' : 'Regular Habits'}
        </h1>
      ...
      <div className="flex-1 px-4 py-3 space-y-3 overflow-y-auto no-scrollbar pb-32">
        {filteredHabits.length > 0 ? (
              ...
              <div className="space-y-3">
                {filteredHabits.map((habit) => (
                  <MyHabitCard
                    key={habit.id}
                    habit={habit}
                    onClick={() => router.push(`/managements/${habit.id}`)}
                  />
                ))}
              </div>
            </SortableContext>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
            ...
            <div>
              <p className="text-gray-500 font-bold">No {defaultTab}s yet</p>
