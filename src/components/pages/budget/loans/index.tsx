...
export default function LoansPage() {
  ...
  const totalLentPending = loans
    .filter((l) => l.type === 'lend' && l.status !== 'repaid')
    .reduce((sum, l) => sum + (l.amount - (l.repaidAmount || 0)), 0);

  const totalBorrowedPending = loans
    .filter((l) => l.type === 'borrow' && l.status !== 'repaid')
    .reduce((sum, l) => sum + (l.amount - (l.repaidAmount || 0)), 0);
      ...
      <div className="w-full max-w-lg mx-auto p-4 space-y-5">
        ...
        <header className="flex justify-between items-center py-1">
          ...
          <div className="text-center">
            <h1 className="text-base font-extrabold text-gray-900 dark:text-white">Loan & Borrow</h1>
            <p className="text-[10px] text-gray-400 font-medium">Track money lent & borrowed</p>
          </div>
          <div className="w-10" />
        </header>
