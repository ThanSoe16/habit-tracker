...
import { Trophy, Dumbbell, Flame, Activity, Calendar, Zap } from 'lucide-react';
import { useGymStore } from '@/store/use-gym-store';
import { cn } from '@/utils/cn';

export default function GymReportsPage() {
  const { history, weeklyPlan, bodyMetricLogs } = useGymStore();
  ...
  const consistencyRate = Math.min(
    100,
    Math.round((completedWorkouts / Math.max(1, activePlanDaysCount * 4)) * 100),
  );
  ...
  const weightDiff =
    latestMetric && firstMetric ? latestMetric.weight_kg - firstMetric.weight_kg : 0;
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-3xl p-5 shadow-xs border border-gray-100 dark:border-zinc-800 space-y-4">
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {weeklyPlan.map((day, idx) => (
            ...
            >
              <span className="text-[10px] font-black uppercase">{day.dayName.slice(0, 3)}</span>
              {day.isRestDay ? (
