  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-zinc-800">
      ...
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-base font-bold text-gray-900 dark:text-white">Habits Completed</h3>
      ...
      <div className="flex gap-1">
        ...
        <div className="flex-1 flex items-end justify-between gap-2">
          {weekData.map((d, i) => {
            ...
            const isHighest = d.completed > 0 && d.completed === highestDay.completed;
                ...
                <div className="relative w-full flex justify-center">
                  ...
                  {d.completed > 0 && (
                    <div
                      className={`absolute -top-7 bg-indigo-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap transition-opacity duration-150 ${
                        isHighest ? 'opacity-100' : 'opacity-0 group-hover/bar:opacity-100'
                      }`}
                    >
                      {d.completed}
                    </div>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      maxHeight: '120px',
                      backgroundColor:
                        d.completed > 0 ? (isHighest ? '#818CF8' : '#C7D2FE') : '#F3F4F6',
                    }}
                  />
                </div>
                <span className="text-[11px] text-muted-foreground font-medium mt-1">{d.day}</span>
              </div>
            );
          })}
        </div>
