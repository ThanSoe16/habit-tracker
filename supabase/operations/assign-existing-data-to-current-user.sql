-- Generated owner-specific rollout; run the WHOLE file in Supabase SQL Editor.
-- Verified owner: f4b7da9e-8a5a-4ee3-9abb-526a26a8d464. This script applies the account migrations.
-- Private database row backups are retained in account_backup_20260912.
...
SET LOCAL app.legacy_owner_id = 'f4b7da9e-8a5a-4ee3-9abb-526a26a8d464';
...
DO $$ BEGIN
  ...
  END IF;
END $$;
CREATE SCHEMA account_backup_20260912;
REVOKE ALL ON SCHEMA account_backup_20260912 FROM PUBLIC, anon, authenticated;
CREATE TABLE account_backup_20260912._record_counts (table_name TEXT PRIMARY KEY, rows_before BIGINT, rows_after BIGINT);
CREATE TABLE account_backup_20260912._schema_metadata (kind TEXT, definition JSONB);
REVOKE ALL ON ALL TABLES IN SCHEMA account_backup_20260912 FROM PUBLIC, anon, authenticated;
  ...
  FOR source_table IN SELECT tablename FROM pg_tables WHERE schemaname = 'public' ORDER BY tablename LOOP
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public'
      ...
      EXECUTE format('SELECT count(*) FROM public.%I WHERE user_id IS NOT NULL AND user_id::text NOT IN (%L, %L)',
        source_table.tablename, 'f4b7da9e-8a5a-4ee3-9abb-526a26a8d464', 'default_user') INTO foreign_owners;
      IF foreign_owners <> 0 THEN RAISE EXCEPTION 'Unexpected owner in %. No data changed.', source_table.tablename; END IF;
    END IF;
    EXECUTE format('CREATE TABLE account_backup_20260912.%I AS TABLE public.%I', source_table.tablename, source_table.tablename);
    EXECUTE format('REVOKE ALL ON account_backup_20260912.%I FROM PUBLIC, anon, authenticated', source_table.tablename);
    EXECUTE format('ALTER TABLE account_backup_20260912.%I ENABLE ROW LEVEL SECURITY', source_table.tablename);
    EXECUTE format('SELECT count(*) FROM public.%I', source_table.tablename) INTO n;
    INSERT INTO account_backup_20260912._record_counts VALUES (source_table.tablename, n, NULL);
  END LOOP;
END $$;
LOCK TABLE storage.objects IN SHARE ROW EXCLUSIVE MODE;
CREATE TABLE account_backup_20260912._storage_objects AS TABLE storage.objects;
CREATE TABLE account_backup_20260912._storage_buckets AS TABLE storage.buckets;
REVOKE ALL ON ALL TABLES IN SCHEMA account_backup_20260912 FROM PUBLIC, anon, authenticated;
INSERT INTO account_backup_20260912._schema_metadata
...
-- BEGIN 20260912000000_account_ownership.sql
...
-- END 20260912000000_account_ownership.sql

-- BEGIN 20260912000001_account_workout_exercises.sql
...
-- END 20260912000001_account_workout_exercises.sql
...
BEGIN
  FOR source_table IN SELECT table_name, rows_before FROM account_backup_20260912._record_counts LOOP
    EXECUTE format('SELECT count(*) FROM public.%I', source_table.table_name) INTO n;
    IF n <> source_table.rows_before THEN RAISE EXCEPTION 'Row count changed in %', source_table.table_name; END IF;
    EXECUTE format(
      'SELECT EXISTS ((SELECT to_jsonb(t) - ''user_id'' FROM account_backup_20260912.%I t EXCEPT ALL SELECT to_jsonb(t) - ''user_id'' FROM public.%I t) UNION ALL (SELECT to_jsonb(t) - ''user_id'' FROM public.%I t EXCEPT ALL SELECT to_jsonb(t) - ''user_id'' FROM account_backup_20260912.%I t))',
      source_table.table_name, source_table.table_name, source_table.table_name, source_table.table_name
    ) INTO different;
    IF different THEN RAISE EXCEPTION 'Original values changed in %. Rolling back.', source_table.table_name; END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public'
      ...
      EXECUTE format('SELECT count(*) FROM public.%I WHERE user_id::text IS DISTINCT FROM %L',
        source_table.table_name, 'f4b7da9e-8a5a-4ee3-9abb-526a26a8d464') INTO foreign_owners;
      IF foreign_owners <> 0 THEN RAISE EXCEPTION 'Owner verification failed in %', source_table.table_name; END IF;
    END IF;
    UPDATE account_backup_20260912._record_counts SET rows_after = n WHERE table_name = source_table.table_name;
  END LOOP;
  IF EXISTS (
    (SELECT to_jsonb(t) - 'owner_id' - 'updated_at' FROM account_backup_20260912._storage_objects t
    ...
    (SELECT to_jsonb(t) - 'owner_id' - 'updated_at' FROM storage.objects t
      EXCEPT ALL SELECT to_jsonb(t) - 'owner_id' - 'updated_at' FROM account_backup_20260912._storage_objects t)
  ) THEN RAISE EXCEPTION 'Storage metadata changed unexpectedly. Rolling back.'; END IF;
END $$;
NOTIFY pgrst, 'reload schema';
COMMIT;
SELECT table_name, rows_before, rows_after FROM account_backup_20260912._record_counts ORDER BY table_name;
